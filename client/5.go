...
func main() {
	...
	newText, tokenToKeywordMap := replaceKeywordsWithTokensAndCreateMap(text, keywords)

	fmt.Println("替换后的文本:", newText)
	fmt.Println("标记到关键字的映射:", tokenToKeywordMap)
}

// replaceKeywordsWithTokensAndCreateMap 替换文本中的关键字，并为每个实例创建特定标记，然后生成标记到关键字的映射
func replaceKeywordsWithTokensAndCreateMap(text string, keywords []string) (string, map[string]string) {
